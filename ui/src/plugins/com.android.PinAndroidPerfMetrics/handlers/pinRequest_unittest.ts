...
describe('normalizePinIntent & parsePinRequests', () => {
  ...
  describe('Specific dictionary records & strict attribute matching', () => {
    it('parses single CUJ dictionary record without default process', () => {
      // Without process specified, only PinCujMetricHandler matches.
      expect(normalizePinIntent({cuj: 'RECENTS_SCROLLING'})).toEqual([
        {kind: PinIntentKind.Cuj, cujName: 'RECENTS_SCROLLING'},
      ]);

      // With process specified, PinCujScopedJank matches.
      expect(
        normalizePinIntent({
          process: 'com.android.systemui',
    ...
    it('parses process memory track requests with or without kind attribute', () => {
      expect(
        normalizePinIntent({
          process: 'com.android.systemui',
          memory: 'heapSize',
  ...
  describe('Priority & exact attribute enforcement (not more, not less)', () => {
    ...
    it('rejects dictionary requests with extra unrecognized attributes', () => {
      expect(
        normalizePinIntent({cuj: 'RECENTS_SCROLLING', unexpectedField: 'foo'}),
      ).toEqual([]);
    });
  });
