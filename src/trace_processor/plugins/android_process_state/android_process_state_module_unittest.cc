...
class AndroidProcessStateModuleTest : public testing::Test {
 public:
  AndroidProcessStateModuleTest() {
    ...
    module_ = std::make_unique<AndroidProcessStateModule>(&module_context_,
                                                          tracker_.get());
  }
...
TEST_F(AndroidProcessStateModuleTest, ParseDumpPopulatesProcessStateTable) {
...
TEST_F(AndroidProcessStateModuleTest, ParseFreezerDump) {
  protozero::HeapBuffered<AndroidFreezerStateSnapshot> freezer_snapshot;
  {
    ...
    rec->set_unfreeze_reason(
        ::com::android::internal::pbzero::UnfreezeReason::UFR_ACTIVITY);
  }
  {
    // Omitted unfreeze_reason should fall back to 0 (UFR_NONE).
    auto* rec = freezer_snapshot->add_record();
    rec->set_pid(4002);
    rec->set_uid(10006);
  }
  ...
  EXPECT_EQ(freezer_state_table_->row_count(), 2u);

  auto rr0 = (*freezer_state_table_)[0];
  EXPECT_EQ(rr0.id().value, 0u);
  EXPECT_FALSE(rr0.ts().has_value());
  EXPECT_EQ(rr0.is_initial(), 1u);
  EXPECT_FALSE(rr0.unfrozen_dur_ms().has_value());
  EXPECT_FALSE(rr0.frozen_dur_ms().has_value());
  ASSERT_TRUE(rr0.unfreeze_reason().has_value());
  EXPECT_STREQ(context_.storage->GetString(*rr0.unfreeze_reason()).c_str(),
               "1");

  auto rr1 = (*freezer_state_table_)[1];
  EXPECT_EQ(rr1.id().value, 1u);
  EXPECT_FALSE(rr1.ts().has_value());
  EXPECT_EQ(rr1.is_initial(), 1u);
  EXPECT_FALSE(rr1.unfrozen_dur_ms().has_value());
  EXPECT_FALSE(rr1.frozen_dur_ms().has_value());
  ASSERT_TRUE(rr1.unfreeze_reason().has_value());
  EXPECT_STREQ(context_.storage->GetString(*rr1.unfreeze_reason()).c_str(),
               "0");
}
